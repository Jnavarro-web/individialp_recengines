...
# In Colab: upload ratings.csv, links.csv alongside the TMDB files
# ratings.csv  → one row per user-movie rating (userId, movieId, rating)
# links.csv    → bridges MovieLens movieId to TMDB id (tmdbId)

ml_ratings = pd.read_csv('/content/ratings.csv')
ml_links   = pd.read_csv('/content/links.csv')

# Clean the tmdbId column (some entries are floats / missing)
...
# links.csv is the bridge: MovieLens movieId → TMDB id
...
# For each user we compute their average rating per genre.
# This answers: does this person like Action? Romance? Horror?
# We use only their actual rating history — no assumptions.

from collections import defaultdict

def build_user_genre_profile(user_df):
    """Returns {genre: mean_rating} built from one user's real rating history."""
    genre_scores = defaultdict(list)
    for _, row in user_df.iterrows():
        for g in row['genres_list']:
            genre_scores[g].append(row['rating'])
    return {g: round(sum(v) / len(v), 3) for g, v in genre_scores.items()}

user_profiles = {}
for uid, grp in ratings_rich.groupby('userId'):
    user_profiles[uid] = build_user_genre_profile(grp)
...
for uid in [1, 5]:
    ...
    for genre, score in sorted(profile.items(), key=lambda x: -x[1]):
        bar = '█' * int(score * 2)
        liked = '✓' if score >= 3.5 else '✗'
        print(f"  {liked} {genre:<20} {score:.2f}  {bar}")
    print()
...
#
# This is the question TMDB alone could never answer.
...
for uid in sample_users:
    ...
    # NP: same global top-10 for everyone (minus already seen)
...
# ── Cell 24 ──────────────────────────────────────────────────
# ── Hybrid Recommender with personal genre hard-filter ────────────────────
# This model goes one step further than CF alone:
#
#   Step 1 — CF: find films similar to what this user has liked
#   Step 2 — Hard filter: remove any film whose primary genre the user
#             has historically rated below 3.0 (they've shown they dislike it)
#   Step 3 — Top up: if not enough films pass the filter, fill remaining
#             slots from the NP global list (same filter applied)
#
# Unknown genres (user has never rated that genre) are ALLOWED through —
# we only block genres the user has actively shown they dislike.

def hybrid_recommend(user_id, n=10, dislike_threshold=3.0):
    profile = user_profiles.get(user_id, {})
    seen    = set(ratings_rich[ratings_rich.userId == user_id]['tmdbId'].tolist())

    def passes_filter(tmdb_id):
        """True if this movie's primary genre is not disliked by the user."""
        genres = movie_genres_lookup.get(tmdb_id, [])
        if not genres:
            return True
        score = profile.get(genres[0])
        if score is None:
            return True        # never rated this genre → give it a chance
        return score >= dislike_threshold

    # Step 1: CF candidates from liked history
    liked = ratings_rich[
        (ratings_rich.userId == user_id) & (ratings_rich.rating >= 3.5)
    ]['tmdbId'].tolist()
    liked_in_model = [m for m in liked if m in cf_model.index_map_]
    cf_candidates  = cf_model.recommend(liked_in_model[:5], n=50, seen_ids=list(seen))                      if liked_in_model else []

    # Step 2: apply hard genre filter
    filtered = [m for m in cf_candidates if passes_filter(m)]

    # Step 3: top up from NP global list
    if len(filtered) < n:
        np_candidates = np_model.recommend(n=200, seen_ids=list(seen)).index.tolist()
        for m in np_candidates:
            if m not in filtered and passes_filter(m):
                filtered.append(m)
            if len(filtered) >= n:
                break
...
for uid in sample_users:
    ...
    hybrid_mismatch_rates.append(
        genre_mismatch_rate(h_recs, profile, movie_genres_lookup))
...
print("=" * 56)
print(f"{'Model':<24} {'Mismatch Rate':>14} {'Coverage':>10}")
print("-" * 56)
print(f"{'Non-Personalized':<24} {np.mean(np_mismatch_rates):>13.1%} {np_cov_ext:>10.4f}")
print(f"{'Item-Based CF':<24} {np.mean(cf_mismatch_rates):>13.1%} {cf_cov_ext:>10.4f}")
print(f"{'Hybrid (CF + filter)':<24} {np.mean(hybrid_mismatch_rates):>13.1%} {hybrid_cov:>10.4f}")
print("=" * 56)
print()
print("Mismatch Rate = fraction of recs clashing with user's")
print("real genre history (lower = more personalised).")
print("Coverage = fraction of catalogue reached across all users.")

# ── Cell 26 ──────────────────────────────────────────────────
# ── Visualisation: three-model comparison ────────────────────────────────
models  = ['Non-Personalized', 'Item-Based CF', 'Hybrid']
mis_vals = [np.mean(np_mismatch_rates),
            np.mean(cf_mismatch_rates),
            np.mean(hybrid_mismatch_rates)]
cov_vals = [np_cov_ext, cf_cov_ext, hybrid_cov]
colors   = ['steelblue', 'darkorange', 'seagreen']

fig, axes = plt.subplots(1, 2, figsize=(13, 5))

# Mismatch rate
bars1 = axes[0].bar(models, mis_vals, color=colors, alpha=0.85, edgecolor='white')
axes[0].set_title('Genre Mismatch Rate\n(lower = more personalised)', fontweight='bold')
axes[0].set_ylabel('Fraction of mismatched recommendations')
axes[0].set_ylim(0, max(mis_vals) * 1.3)
for bar, v in zip(bars1, mis_vals):
    axes[0].text(bar.get_x() + bar.get_width()/2, v + 0.005,
                 f'{v:.1%}', ha='center', fontweight='bold', fontsize=10)

# Coverage
bars2 = axes[1].bar(models, cov_vals, color=colors, alpha=0.85, edgecolor='white')
axes[1].set_title('Catalogue Coverage\n(higher = more of catalogue used)', fontweight='bold')
axes[1].set_ylabel('Fraction of catalogue reached')
axes[1].set_ylim(0, max(cov_vals) * 1.3)
for bar, v in zip(bars2, cov_vals):
    axes[1].text(bar.get_x() + bar.get_width()/2, v + 0.001,
                 f'{v:.3f}', ha='center', fontweight='bold', fontsize=10)

plt.tight_layout()
plt.savefig('hybrid_comparison.png', bbox_inches='tight')
plt.show()
print("Chart saved ✓")
